/**
 * Decode route module
 * @module route
 */

import express from "express";
import { decode } from "./api"
import { IDecodeAPIResults } from "./types";

const route = express.Router()

/**
 * function that handle the decode API call and sends the result back using response object
 * @async
 * @function processDecoding
 * @param {express.Request} req - Express's request object
 * @param {express.Response} res - Express's response object
 * @returns {Promise<void>} - returns void
 */
const processDecoding = async (req: express.Request, res: express.Response): Promise<void> => {
    const response: IDecodeAPIResults = await decode({encodedString: req.params.encodedString})
    res.status(response.status).json(response.results)
}

route.get('/:encodedString', processDecoding)

export default route